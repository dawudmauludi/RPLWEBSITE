        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border-l-4 border-purple-500">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center space-x-3 mb-4 md:mb-0">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Data master_image</h1>
                        <p class="text-gray-600 mt-1">Kelola data master_image dengan sistem</p>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                    <form method="GET" action="{{ route('admin.master_image.index') }}" id="searchForm" autocomplete="off" class="w-full sm:w-auto">
                        <div class="relative">
                    ...
                    <a href="{{ route('admin.master_image.create') }}"
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div id="guruTable">
                <div class="overflow-x-auto">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-purple-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-600 font-mono">{{ $master_image->type }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-2">
                                        <img src="{{ $master_image->image ? url('storage/' . $master_image->image) : asset('images/no-image.png') }}"
                                             alt="{{ $master_image->name }}"
                                             class="w-20 h-20 object-cover rounded-lg">
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('admin.master_image.edit', $master_image->id) }}"
                                        ...
                                        <form action="{{ route('admin.master_image.destroy', $master_image->id) }}" method="POST" class="inline delete-user-form">
                                            @csrf @method('DELETE')
                                            <button type="button"
                    ...
                    @if($master_images->isEmpty())
                    <div class="text-center py-12">
                        <div class="flex flex-col items-center">
                            <div class="bg-purple-100 p-6 rounded-full mb-4">
                                <i data-feather="users" class="w-12 h-12 text-purple-600"></i>
                            </div>
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">Tidak ada data master_image</h3>
                            <p class="text-gray-600 mb-4">Belum ada master_image yang terdaftar dalam sistem</p>
                            <a href="{{ route('admin.master_image.create') }}"
                               class="inline-flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                                ...
                                <span>Tambah master_image Pertama</span>
                            </a>
                        </div>
                    </div>
                    @endif
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    this.delay = setTimeout(() => {
        ...
        fetch(`{{ route('admin.master_image.index') }}?search=${encodeURIComponent(searchValue)}`, {
            headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            Swal.fire({
                title: 'Yakin hapus?',
                text: `Apakah Anda yakin ingin menghapus data master_image ${nama}?`,
                icon: 'warning',
