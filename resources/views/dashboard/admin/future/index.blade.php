        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border-l-4 border-purple-500">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center space-x-3 mb-4 md:mb-0">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Daftar Future</h1>
                        <p class="text-gray-600 mt-1">Kelola data Futur dengan sistem</p>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                    <form method="GET" action="{{ route('admin.future.index') }}" id="searchForm" autocomplete="off" class="w-full sm:w-auto">
                        <div class="relative">
                    ...
                    <a href="{{ route('admin.future.create') }}"
                ...
                <div class="overflow-x-auto">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-purple-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-600 font-mono">{{ $future->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-600 font-mono">{{ $future->icon }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('admin.future.edit', $future->id) }}"
                                        ...
                                        <form action="{{ route('admin.future.destroy', $future->id) }}" method="POST" class="inline delete-user-form">
                                            @csrf @method('DELETE')
                                            <button type="button"
                    ...
                    <div class="text-center py-12">
                        <div class="flex flex-col items-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">Tidak ada data Future</h3>
                            <p class="text-gray-600 mb-4">Belum ada future yang terdaftar dalam sistem</p>
                            <a href="{{ route('admin.future.create') }}"
                               class="inline-flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                                ...
                                <span>Tambah Kategori Future Pertama</span>
                            </a>
                        </div>
                    </div>
                    @endif
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    this.delay = setTimeout(() => {
        ...
        fetch(`{{ route('admin.future.index') }}?search=${encodeURIComponent(searchValue)}`, {
            headers: {
